...
def parse_library(text):
    ...
    category_map = {
        ...
        'COG': 'mind',
        'STOIC': 'spirit',
        'LOG': 'business' # Mapping LOG to business/asset as closest match
    }

    icon_map = {
        'BIO': 'Activity',
        'PHYS': 'Dumbbell',
        'FUEL': 'Apple', # Will map to something available
        'COG': 'Brain',
        'STOIC': 'Flame',
        'LOG': 'Clipboard'
    }
    ...
    for line in lines:
        ...
        if re.match(r'^[A-Z]+_\d+:', line):
            ...
            # Determine config based on keywords
            freq_type = 'ABSOLUTE'
            # Weekly items logic: Lift, Sauna, Cardio, Sport, Ruck, Yoga
            ...
            # Special Overrides
            if 'Cold' in title: freq_type = 'ABSOLUTE' # Plunge usually daily? Or Freq?
            # Re-read prompt: "Set frequency_type = 'FREQUENCY' for weekly items (Lift, Sauna, Cardio)."
            # "Set frequency_type = 'ABSOLUTE' for daily items (Sleep, Sun, Water)."
        ...
        if current_habit:
            if line.startswith('Target:'):
                ...
                current_habit['lenses']['FORTITUDE'] = line.replace('Fortitude:', '').strip().strip('"')
            elif line.startswith('Reason:'):
                current_habit['lenses']['REASON'] = line.replace('Reason:', '').strip().strip('"')
            elif line.startswith('Visionary:'):
                current_habit['lenses']['VISIONARY'] = line.replace('Visionary:', '').strip().strip('"')
            elif line.startswith('Analytical:'):
                current_habit['lenses']['ANALYTICAL'] = line.replace('Analytical:', '').strip().strip('"')
...
def generate_sql(habits):
    sql = ["TRUNCATE habits;"]

    # Core habits active by default (~10)
    core_ids = ['BIO_01', 'BIO_02', 'BIO_05', 'PHYS_01', 'PHYS_10', 'FUEL_01', 'COG_01', 'STOIC_04', 'LOG_01', 'LOG_03']

    for h in habits:
        status = 'active' if h['id'] in core_ids else 'archived'
        metadata = json.dumps({
        ...
        # INSERT
        # Assuming table schema: habit_id, title, description, status, metadata
        stmt = f"INSERT INTO habits (habit_id, title, description, status, metadata) VALUES ('{h['id']}', '{title}', '{desc}', '{status}', '{metadata}');"
        sql.append(stmt)

    return "\n".join(sql)

def generate_ts(habits):
    # We need to preserve the PROTOCOL_STACKS and ICON_MAP, so we'll just generate the array part or full file.
    # It's safer to generate the full file content with placeholders or standard data.
...
if __name__ == "__main__":
    ...
    # 1. SQL
    sql_out = generate_sql(habits)
    os.makedirs('apps/maximost-api/src/scripts', exist_ok=True)
    with open('apps/maximost-api/src/scripts/reset_db_final.sql', 'w') as f:
        f.write(sql_out)
