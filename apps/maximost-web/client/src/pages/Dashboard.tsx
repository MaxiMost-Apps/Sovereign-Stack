...
export default function Dashboard() {
  ...
  const activeHabits = library.map(master => {
    ...
    return { ...master, ...userState, default_config: { ...master.default_config, ...userState.metadata?.config }, visuals: { ...master.visuals, ...userState.metadata?.visuals }, status: userState.status || 'active', is_completed: userState.status === 'completed' };
  }).filter(Boolean);
        ...
        <div className="max-w-3xl mx-auto flex items-center justify-between">
          <div>
            <div className="flex items-center gap-2">
                ...
                <span className="bg-blue-600 text-white text-[9px] font-bold px-1.5 py-0.5 rounded">V2</span>
            </div>
            <p className="text-[10px] text-blue-500 font-bold tracking-widest uppercase mt-1">
                 {new Date().toLocaleDateString('en-US', { weekday: 'long', month: 'short', day: 'numeric' })}
            </p>
          </div>
          <div className="flex gap-2">
             {view === 'day' && <button onClick={() => setIsReordering(!isReordering)} className={`p-2 rounded-lg transition-colors ${isReordering ? 'text-blue-400 bg-blue-900/20' : 'text-slate-600 hover:text-white'}`}><ArrowUpDown size={20} /></button>}
             <button onClick={() => setIsLocked(!isLocked)} className={`p-2 rounded-lg transition-colors ${isLocked ? 'text-red-500 bg-red-900/20' : 'text-slate-600 hover:text-white'}`}>{isLocked ? <Lock size={20} /> : <Unlock size={20} />}</button>
          </div>
        </div>
      </div>

      <div className="max-w-3xl mx-auto p-4 mt-4">
        <div className="flex bg-[#0A0F1C] p-1 rounded-xl border border-white/5 mb-8">
           {[{ id: 'day', icon: LayoutGrid, label: 'DAY' }, { id: 'week', icon: Calendar, label: 'WEEK' }, { id: 'month', icon: BarChart3, label: 'MONTH' }].map(tab => (
            <button key={tab.id} onClick={() => setView(tab.id as any)} className={`flex-1 py-3 text-[10px] font-black tracking-widest ${view === tab.id ? 'bg-slate-800 text-white rounded-lg shadow-lg' : 'text-slate-500'}`}>{tab.label}</button>
           ))}
        </div>
