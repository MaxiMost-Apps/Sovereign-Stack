...
import { Send, Settings, Brain, Trash2, Info } from 'lucide-react';
...
export default function AICoachPage() {
  ...
  useEffect(() => {
    if(persona && COACH_CONFIG[persona]) {
      setMessages(prev => [...prev, {
         id: Date.now(),
         text: `SYSTEM: Coach switched to ${COACH_CONFIG[persona].name}.`,
         sender: 'system'
      }]);
    }
  }, [persona]);
  ...
  useEffect(() => {
    ...
    const load = async () => {
       ...
       if(history?.length) {
          ...
          setMessages([{id:'init', text:`I am ${currentCoach.name}. ${currentCoach.desc}`, sender:'ai'}]);
       }
  ...
  const handleSend = async (e) => {
    ...
    DIRECTIVE:
    Use this data to audit the user. If trend is 'down' on critical habits, flag it as a crack in the foundation.
  ...
  return (
      ...
      <div className="flex flex-col h-[calc(100vh-100px)] max-w-5xl mx-auto gap-4">
        {/* HEADER */}
        <div className={`bg-gray-900 border-2 ${activeCoach?.colors?.border} rounded-2xl p-6 flex items-center justify-between relative overflow-hidden`}>
           ...
           <div className="flex items-center gap-6 z-10">
              ...
              <div>
                 <h1 className="text-3xl font-black text-white uppercase tracking-tighter">{activeCoach?.name}</h1>
           ...
           <button onClick={() => navigate('/settings')} className="z-10 text-xs bg-black/40 px-4 py-2 rounded-lg border border-white/5 hover:bg-black/60 flex items-center gap-2 transition-all">
              <Settings size={14} /> Change Persona
           </button>
        </div>
        ...
        <div className="flex flex-1 gap-4 overflow-hidden">
           ...
           <div className="flex-1 flex flex-col bg-gray-900/50 backdrop-blur rounded-2xl border border-gray-800 overflow-hidden relative">
              ...
              <form onSubmit={handleSend} className="p-4 bg-black/40 border-t border-white/5 flex gap-3">
                <input value={input} onChange={e => setInput(e.target.value)} className="flex-1 bg-gray-800/50 border border-gray-700 rounded-xl px-4 py-3 text-white focus:outline-none focus:ring-1 focus:ring-blue-500/50" placeholder={`Message ${activeCoach.name}...`} />
                <button type="submit" disabled={loading} className="bg-blue-600 p-3 rounded-xl text-white hover:scale-105"><Send size={20}/></button>
              </form>
           </div>

           {/* MEMORY SIDEBAR */}
           <div className="w-80 bg-gray-900/30 backdrop-blur rounded-2xl border border-gray-800 flex flex-col hidden md:flex">
              <div className="p-4 border-b border-gray-800 flex items-center gap-2 text-gray-400">
                 <Brain size={16} /> <h3 className="font-bold text-xs uppercase tracking-widest">Glass Box Memory</h3>
              </div>
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-2">
                 ...
                 <div className="p-4 text-[10px] text-gray-600 text-center leading-relaxed">
                    <Info size={12} className="mx-auto mb-2"/>
                    This is everything the AI knows about you. Delete anything inaccurate.
                 </div>
              </div>
           </div>
        </div>
