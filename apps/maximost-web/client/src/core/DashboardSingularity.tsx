...
import { HabitSlideOver } from '@/components/habits/HabitSlideOver';
...
export default function DashboardSingularity() {
  ...
  // MERGE LOGIC (Decoupled from Profile Lenses)
  const activeHabits = library.map(master => {
    ...
    return {
      ...master,
      ...userState,
      // Prefer user config, fallback to master
        ...
        <div className="max-w-3xl mx-auto flex items-center justify-between">
          <div>
            ...
            <div className="flex items-center gap-3 mt-1">
               <button className="text-slate-600 hover:text-white p-1"><ChevronLeft size={16}/></button>
               <p className="text-[10px] text-blue-500 font-bold tracking-widest uppercase">
                 {new Date().toLocaleDateString('en-US', { weekday: 'long', month: 'short', day: 'numeric' })}
               </p>
               <button className="text-slate-600 hover:text-white p-1"><ChevronRight size={16}/></button>
            </div>
          </div>
        ...
        {view === 'day' && (
          <div className="space-y-4 animate-in fade-in">
            ...
            <div className="space-y-3">
                ...
                {absoluteHabits.map((h, i) => <DailyHabitRow key={h.id} habit={h} index={i} isReordering={isReordering} isLocked={isLocked} onToggle={toggleHabit} onOpenInfo={() => setDrawerHabit(h)} onOpenEdit={() => setSlideOver({ habit: h, mode: 'EDIT' })} />)}
            </div>
            {/* FREQUENCY */}
            <div className="space-y-3 pt-6">
                ...
                {frequencyHabits.map((h, i) => <DailyHabitRow key={h.id} habit={h} index={i} isReordering={isReordering} isLocked={isLocked} onToggle={toggleHabit} onOpenInfo={() => setDrawerHabit(h)} onOpenEdit={() => setSlideOver({ habit: h, mode: 'EDIT' })} />)}
            </div>
            {/* CREATE & LIBRARY */}
            <div className="pt-8">
              <button onClick={() => setSlideOver({ habit: { title: '', visuals: { color: 'bg-blue-500', icon: 'Zap' }, default_config: {} }, mode: 'CREATE' })} className="w-full py-4 bg-blue-600 hover:bg-blue-500 rounded-2xl flex items-center justify-center gap-2 text-white font-bold uppercase tracking-widest transition-all shadow-lg shadow-blue-900/20"><Plus size={16} /> Initialize New Habit</button>
            </div>
            <div className="mt-20 border-t border-white/5 pt-10"><HabitLibrary onDeploy={(h) => setSlideOver({ habit: h, mode: 'CREATE' })} /></div>
          </div>
        )}
      ...
      <AnimatePresence>
        ...
        {slideOver && (
          <HabitSlideOver
            ...
            onConfirm={(updated) => {
                if (slideOver.mode === 'CREATE') toggleHabit(updated.habit_id || updated.id);
                else updateHabitConfig(updated.id || updated.habit_id, updated);
                setSlideOver(null);
            }}
          />
        )}
