...
import { Lock, Unlock, ArrowUpDown, Plus, Check, MoreVertical, Edit2, Trash2, Info, Flame, X, GripVertical, Activity, Zap, Brain, Droplet, Moon, Sun, Coffee, Dumbbell, Book, Briefcase, Heart, Calendar, FileText, ArrowRight } from 'lucide-react';
...
function DailyHabitRow({ habit, isCompleted, logEntry, onToggle, onEdit, onDelete, isSystemLocked, isSortMode, date }: any) {
  ...
  return (
    <div className={cn("group relative flex items-center justify-between p-4 rounded-xl border transition-all mb-3 bg-[#0b0c10] border-white/5", isFullyComplete ? "opacity-100" : "opacity-90")}
    style={{ height: '80px', borderColor: isFullyComplete ? primaryColor : (isSortMode ? '#1e3a8a' : 'rgba(255,255,255,0.05)'), boxShadow: isFullyComplete ? `0 0 15px -10px ${primaryColor}` : 'none' }}>

      <div className="flex items-center gap-4 flex-1">
        {isSortMode && <div className="text-slate-500 cursor-grab active:cursor-grabbing"><GripVertical size={24} /></div>}
        <div className="w-12 h-12 rounded-xl flex items-center justify-center transition-all bg-white/5" style={{ color: primaryColor }}><IconComponent size={24} /></div>
        <div className="flex flex-col gap-1">
            <div className="flex items-center gap-3">
                ...
                <div className="relative">
                    <button onClick={() => setShowMenu(!showMenu)} className="p-1 text-slate-600 hover:text-white"><MoreVertical size={16} /></button>
                    {showMenu && (
      ...
      <div className="pl-4 border-l border-white/5 relative h-12 flex items-center">
           <div className="relative flex items-center justify-center">
               <button onClick={() => !isSystemLocked && (isQuantified ? setShowInput(true) : onToggle(habit.id, date, isFullyComplete ? 0 : 1))}
                 ...
                 style={{ borderColor: isFullyComplete ? primaryColor : '#334155', backgroundColor: isFullyComplete ? primaryColor : 'transparent', boxShadow: isFullyComplete ? `0 0 15px ${primaryColor}` : 'none', opacity: isSystemLocked ? 0.5 : 1 }}>
  ...
  return (
    <form onSubmit={(e) => { e.preventDefault(); onSubmit({ ...initialData, ...formData, daily_goal: parseInt(String(formData.daily_goal)) || 1 }); }} className="space-y-6 text-slate-200">
      <div className="grid grid-cols-2 gap-4">
        <div><label className="text-[10px] font-black text-slate-500 uppercase tracking-widest block mb-2">Frequency</label>
            <select value={formData.frequency_type} onChange={(e) => setFormData({ ...formData, frequency_type: e.target.value })} className="w-full bg-[#0b0c10] border border-white/10 rounded-lg p-3 text-white text-xs font-bold uppercase"><option value="daily">Daily</option><option value="weekly">Weekly</option></select></div>
        <div><label className="text-[10px] font-black text-slate-500 uppercase tracking-widest block mb-2">Start Date</label>
            <input type="date" value={formData.start_date} onChange={(e) => setFormData({ ...formData, start_date: e.target.value })} className="w-full bg-[#0b0c10] border border-white/10 rounded-lg p-3 text-white text-xs font-bold uppercase" /></div>
      </div>
      <input type="text" value={formData.title} onChange={(e) => setFormData({ ...formData, title: e.target.value })} placeholder="Habit Name" className="w-full bg-[#0b0c10] border border-white/10 rounded-xl p-4 text-white font-black text-xl focus:border-blue-500 outline-none" required autoFocus />
      <div className="grid grid-cols-2 gap-4">
          <div><label className="text-xs text-slate-500 font-bold block mb-2">Target</label><input type="number" min="1" value={formData.daily_goal} onChange={(e) => setFormData({ ...formData, daily_goal: e.target.value })} className="w-full bg-[#0b0c10] border border-white/10 rounded-lg p-3 text-white font-mono" /></div>
          <div><label className="text-xs text-slate-500 font-bold block mb-2">Unit</label><input type="text" value={formData.unit} onChange={(e) => setFormData({ ...formData, unit: e.target.value })} className="w-full bg-[#0b0c10] border border-white/10 rounded-lg p-3 text-white" /></div>
      </div>
      <div className="grid grid-cols-12 gap-6 pt-4 border-t border-white/5">
          <div className="col-span-4 border-r border-white/5 pr-4">
             ...
             <div className="grid grid-cols-3 gap-3">
                {COLORS.map(c => ( <button key={c} type="button" onClick={() => setFormData({ ...formData, color: c })} className={`w-8 h-8 rounded-full transition-all ${formData.color === c ? 'ring-2 ring-white scale-110' : 'opacity-40'}`} style={{ backgroundColor: getThemeStyles(c).primary }} /> ))}
             </div>
          </div>
          <div className="col-span-8">
             <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest block mb-3">Visual Anchor</label>
             <div className="grid grid-cols-6 gap-2">
                {ICONS.map(({ id, icon: Icon }) => ( <button key={id} type="button" onClick={() => setFormData({ ...formData, icon: id })} className={`p-2 rounded-lg flex items-center justify-center transition-all ${formData.icon === id ? 'bg-blue-600 text-white' : 'text-slate-600 hover:text-white'}`}><Icon size={18} /></button> ))}
             </div>
          </div>
      </div>
      <div className="flex gap-3 pt-6 border-t border-white/5 items-center sticky bottom-0 bg-[#14151a]">
        ...
        <button type="submit" className="flex-1 py-3 bg-blue-600 hover:bg-blue-500 text-white rounded-xl font-bold flex items-center justify-center gap-2 uppercase text-xs tracking-widest"><Save size={16} /> Save</button>
      </div>
    </form>
  );
}
...
export default function DashboardSingularity() {
  ...
  if (authLoading) return <div className="bg-black h-screen text-white flex items-center justify-center font-bold tracking-widest">LOADING SINGULARITY...</div>;
      ...
      <div className="flex flex-col gap-4 pb-[120px] relative">
        ...
        {isModalOpen && (
           <ConsoleOverlay isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title={editingHabit ? "EDIT" : "NEW"}>
               <InlineHabitForm initialData={initialForm} onSubmit={async (data: any) => { setIsModalOpen(false); await supabase.from('habits').upsert(data); fetchData(); }} onCancel={() => setIsModalOpen(false)} mode={editingHabit ? 'edit' : 'create'} />
           </ConsoleOverlay>
        )}
      </div>
    </Inspector>
