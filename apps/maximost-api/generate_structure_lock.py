...
def generate_sql():
    # Target 'maximost_library_habits' as the Library based on codebase context
    table_name = "maximost_library_habits"

    print(f"-- Migration: 003_structure_lock.sql")
    print("-- Schema Hardening & Sovereign 60 Reset")
    print("")

    print("-- 1. USER HABITS TABLE: Ensure Support for Frequency/Colors")
    print("ALTER TABLE habits ADD COLUMN IF NOT EXISTS frequency TEXT DEFAULT 'daily';")
    print("ALTER TABLE habits ADD COLUMN IF NOT EXISTS base_color TEXT DEFAULT '#6366F1';")
    print("ALTER TABLE habits ADD COLUMN IF NOT EXISTS metadata JSONB DEFAULT '{}'::jsonb;")
    print("ALTER TABLE habits ADD COLUMN IF NOT EXISTS target_value INTEGER DEFAULT 1;")
    print("ALTER TABLE habits ADD COLUMN IF NOT EXISTS daily_goal INTEGER DEFAULT 1;")
    print("ALTER TABLE habits ADD COLUMN IF NOT EXISTS unit TEXT DEFAULT 'rep';")
    print("ALTER TABLE habits ADD COLUMN IF NOT EXISTS atom_id TEXT;")
    print("ALTER TABLE habits ADD COLUMN IF NOT EXISTS title TEXT;")
    print("ALTER TABLE habits ADD COLUMN IF NOT EXISTS slug TEXT;")
    print("")

    print(f"-- 2. LIBRARY TABLE: Ensure Support")
    ...
    print(f"ALTER TABLE {table_name} ADD COLUMN IF NOT EXISTS frequency TEXT DEFAULT 'daily';")
    print("")

    print(f"-- 3. LIBRARY RESET: Truncate to fix ghost IDs")
    print(f"TRUNCATE TABLE {table_name} CASCADE;")
    print("")

    print(f"-- 4. SEED SOVEREIGN 60")
    for atom in ATOMS:
        slug = atom['slug']
        ...
        base_color = atom.get('base_color', '#6366F1')
        logic = atom.get('logic', '').replace("'", "''")
        atom_id = atom.get('atom_id', slug)

        # Use simple INSERT (No conflict needed after Truncate, but good for robustness if re-run)
        print(f"""
