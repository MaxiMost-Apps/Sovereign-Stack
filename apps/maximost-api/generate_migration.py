...
def generate_sql():
    # Note: We target 'library_habits' as primary guess, but users can search-replace to 'maximost_library_habits'
    table_name = "maximost_library_habits" # Defaulting to the one in code

    print(f"-- Migration: Seed Sovereign 60 into {table_name}")
    ...
    for atom in ATOMS:
        slug = atom['slug']
        # Map fields
        # Using 'name' for title as per previous patterns
        ...
        # Assuming schema: slug, name, category, icon, theme, metadata
        # We try to fill common columns

        print(f"""
INSERT INTO {table_name} (slug, name, category, icon, theme, metadata)
VALUES ('{slug}', '{name}', '{category}', '{icon}', '{theme}', '{metadata_json}'::jsonb)
